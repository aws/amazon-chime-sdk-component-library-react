...
import { ChatBubbleContainerProps, MessageVariant } from './ChatBubbleContainer';

export interface ChatBubbleProps extends ChatBubbleContainerProps, Omit<HTMLAttributes<HTMLDivElement>, 'css'>, BaseProps {
...
export const ChatBubble: FC<ChatBubbleProps> = ({ variant, senderName, lastUpdatedTimestamp, content, showName = true, showTail, redacted }) => {
  return (
    <StyledChatBubble variant={variant} editable={false}>
      {showName && <div className='ch-sender-name'>{senderName}</div>}
      <div>{content}</div>
      {lastUpdatedTimestamp && !redacted &&<div className='ch-edited'>{`(edited ${formatDate(lastUpdatedTimestamp)} at ${formatTime(lastUpdatedTimestamp)})`}</div>}
      {showTail &&   
      <svg viewBox="0 0 4 9">
