...
export interface EditableChatBubbleProps extends ChatBubbleContainerProps, Omit<HTMLAttributes<HTMLDivElement>, 'css'>, BaseProps {
...
export const EditableChatBubble: FC<EditableChatBubbleProps> = (props) => {
  const { showName = true, variant, senderName, content, showTail, cancel, cancelLabel = 'Cancel', save, saveLabel = 'Save' } = props;
  ...
  return (
    <StyledChatBubble  variant={variant} editable>
        {showName && <div className='ch-sender-name'>{senderName}</div>}
          <form>
            <Input onChange={handleChange} value={text} showClear={false} ref={inputEl} />
            <div className='ch-edit-buttons'>
              <SecondaryButton label={cancelLabel} onClick={cancel} />
              <PrimaryButton
                label={saveLabel}
