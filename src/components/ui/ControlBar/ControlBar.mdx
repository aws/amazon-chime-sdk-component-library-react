...
The ControlBar component displays a section of all controls with icons and labels.

The component can be placed within the parent element according to the `layout` prop, 
e.g. `top`, `bottom`, `right`, `left`, `undocked-horizontal` and `undocked-vertical`.

The ControlBar component contains one or more `ControlBarItem` components, which can show a label and icon. 
A dropdown list can be added to each ControlBarItem by setting the [popOver props](/?path=/docs/ui-components-popover--basic-pop-over-menu).
