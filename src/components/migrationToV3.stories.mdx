...
```tsx
// Take `CameraSelection` as an example.
...
const App = () => {
  return (
    <MeetingProvider>
      ...
      <CameraSelection appendSampleDevices={true} />
    </MeetingProvider>
  );
};
```
...
```jsx
// Take `useVideoInputs` as an example.
...
const App = () => {
  return (
    <MeetingProvider>
      {/* In V3, `CameraSelection` no longer supports preset device selection options. */}
      <CameraSelection />
    </MeetingProvider>
  );
};
