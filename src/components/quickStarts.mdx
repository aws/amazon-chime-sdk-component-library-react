...
## Set Up Devices Before Joining a Meeting

`MeetingProvider` accepts an optional `deviceController` prop for setting up devices before joining a meeting. When you pass a device controller, device enumeration, selection, camera preview, the microphone activity meter, and permission prompts work before `MeetingManager.join()`. The same controller then carries into the meeting and is not destroyed across `MeetingManager.leave()` and `MeetingManager.join()`, so your device selections persist when you leave and rejoin. The media inputs are stopped on `MeetingManager.leave()` (releasing the camera and microphone), so restart them on rejoin (for example, with `startVideoInput`).

Construct a `DefaultDeviceController` from the Amazon Chime SDK for JavaScript and pass it to `MeetingProvider`. Your application owns the controller: create it and destroy it yourself.

To receive device and meeting events, provide an optional `eventController`. Pass one to `MeetingManager.join()` as shown in the client event ingestion example below, or construct the device controller with your own `EventController` implementation to also capture device events before the meeting.
...
const App = () => {
  ...
  // Destroy the controller you created when the component unmounts.
  useEffect(() => () => { void deviceController.destroy(); }, [deviceController]);
...
// A pre-meeting device-setup page. These components read the provided device controller through the
// `MeetingProvider`, so they enumerate devices, select them, and render a camera preview before
// `MeetingManager.join()` is ever called.
const DeviceSetup = () => (
  <div>
    <MicSelection />
    <SpeakerSelection />
...
`MeetingProvider` reads `deviceController` once, when it creates its `MeetingManager`, so provide the controller before `MeetingProvider` first mounts.

When you omit the `deviceController` prop, `MeetingManager.join()` creates a device controller internally and destroys it on `MeetingManager.leave()`.

To build a pre-meeting device UI, use [`useDeviceController`](?path=/docs/sdk-hooks-usedevicecontroller--page) to access the controller, [`MicSelection`](?path=/docs/sdk-components-deviceselection-micselection--page), [`SpeakerSelection`](?path=/docs/sdk-components-deviceselection-speakerselection--page), and [`CameraSelection`](?path=/docs/sdk-components-deviceselection-camera-cameraselection--page) to select devices, [`PreviewVideo`](?path=/docs/sdk-components-previewvideo--page) to render a camera preview, and [`useLocalAudioInputActivity`](?path=/docs/sdk-hooks-uselocalaudioinputactivity--page) to render a microphone activity meter.
