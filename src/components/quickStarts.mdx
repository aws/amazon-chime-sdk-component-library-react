...
`MeetingProvider` accepts an optional `deviceController` prop for setting up devices before joining a meeting. When you pass a device controller, device enumeration, selection, camera preview, the microphone activity meter, and permission prompts work before `MeetingManager.join()`. The same controller then carries into the meeting, and it is not destroyed across `MeetingManager.leave()` and `MeetingManager.join()`, so selected devices persist when you leave and rejoin.
