...
When `selectAudioInputDevice`, `selectVideoInputDevice`, and `selectAudioOutputDevice` methods of `MeetingManager` failed, they will throw the error directly. The device selection functions return by `useSelectAudioInputDevice`, `useSelectVideoInputDevice`, and `useSelectAudioOutputDevice` hook are built on top of these `MeetingManager` methods, thus they will throw error when failed as well.
...
You can enable posting your logs to a URL by using Amazon Chime SDK for JavaScript [`MeetingSessionPostLogger`](https://aws.github.io/amazon-chime-sdk-js/classes/meetingsessionpostlogger.html). You can pass `logLevel` and `postLogConfig` as props to the `MeetingProvider`.
...
> We post the JS SDK logs to AWS CloudWatch once the demo is deployed. Check the serverless handlers [log](https://github.com/aws-samples/amazon-chime-sdk/tree/main/apps/meeting/serverless/src/handlers.js#L252)
> function and AWS CloudWatch [access policy](https://github.com/aws-samples/amazon-chime-sdk/tree/main/apps/meeting/serverless/template.yaml#L49) in our serverless template for more details.
