<Meta title="SDK Providers/LocalVideoProvider" />

# LocalVideoProvider

Provides the local video status, video tile id associated with the local user's video and a function to toggle their video.

## State

```javascript
{
  // The tile ID associated with the local user's video
  tileId: null | number;

  // Whether or not the local user has video enabled 
  isVideoEnabled: boolean;

  // A function that toggles the local user's video
  toggleVideo: () => Promise<void>;
}
```

You can access the provided values using [useLocalVideo](/?path=/docs/sdk-hooks-uselocalvideo--page) hook.

## Usage

If you are using `MeetingProvider`, `LocalVideoProvider` is rendered by default.

```jsx
import React from 'react';
import { MeetingProvider, useLocalVideo } from 'amazon-chime-sdk-component-library-react';

const App = () => (
  <MeetingProvider>
    <MyChild />
  </MeetingProvider>
);

const MyChild = () => {
  const { isVideoEnabled, toggleVideo } = useLocalVideo();

  return (
    <button onClick={toggleVideo}>
      { isVideoEnabled ? 'Stop your video' : 'Start your video' }
    </button>
  );
}
```

### Dependencies
- `MeetingProvider`
