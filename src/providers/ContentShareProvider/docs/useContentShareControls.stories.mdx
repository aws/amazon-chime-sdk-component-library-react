...
```javascript
{
  ...
  // A function to toggle video file share.
  toggleVideoFileShare: (mediaUrl?: string) => void;
...
const MyChild = () => {
  const { toggleContentShare, toggleVideoFileShare } = useContentShareControls();
  ...
  return (
    ...
    <button onClick={() => toggleVideoFileShare(url)}>Toggle video file share</button>
  );
};
```
