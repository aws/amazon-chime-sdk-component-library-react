...
> **Moved to `DeviceProvider`.** As part of the standalone device layer, `onDeviceReplacement` is now a
> prop on `DeviceProvider` (which owns the device state), not on `MeetingProvider`/`DevicesProvider`.
> Pass it to `DeviceProvider` instead. The behavior below is unchanged; only the prop's location moved.
...
This is accomplished by exposing a `onDeviceReplacement` prop on `DeviceProvider`, allowing your application to customize the behavior of this device reselection step.
