...
Pass a device controller to set up devices before joining a meeting. When provided, device enumeration, selection, camera preview, the microphone activity meter, and permission prompts work before `join()`. The same controller then carries into the meeting, and it is not destroyed across `leave()` and `join()`, so selected devices persist when you leave and rejoin. Construct it (for example, `new DefaultDeviceController(logger)` from the Amazon Chime SDK for JavaScript) and destroy it in your application; the library uses the controller you pass but does not destroy it.
