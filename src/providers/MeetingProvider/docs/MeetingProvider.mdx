...
Pass a device controller to set up devices before joining a meeting. When provided, device enumeration, selection, camera preview, the microphone activity meter, and permission prompts work before `join()`. The same controller then carries into the meeting and is not destroyed across `leave()` and `join()`, so your device selections persist when you leave and rejoin. The media inputs are stopped on `leave()` (releasing the camera and microphone), so restart them on rejoin (for example, with `startVideoInput`). Construct it (for example, `new DefaultDeviceController(logger)` from the Amazon Chime SDK for JavaScript) and destroy it in your application; the library uses the controller you pass but does not destroy it.

`MeetingProvider` reads `deviceController` once, when it creates its `MeetingManager`, so provide the controller before `MeetingProvider` first mounts. When omitted, `join()` creates a device controller internally and destroys it on `leave()`.
