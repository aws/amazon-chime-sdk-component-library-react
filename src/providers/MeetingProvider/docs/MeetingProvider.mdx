...
Pass a device controller to set up devices before joining a meeting. When provided, device enumeration, selection, camera preview, the microphone activity meter, and permission prompts work before `join()`, and the same controller carries into the meeting so selected devices persist across leaving and rejoining. Construct it (for example, `new DefaultDeviceController(logger, { enableWebAudio })` from the Amazon Chime SDK for JavaScript) and destroy it in your application; the library uses the controller you pass but does not create or destroy it.

`MeetingProvider` reads `deviceController` once, when it creates its `MeetingManager`. Provide the controller before `MeetingProvider` first mounts; to change or clear it, remount `MeetingProvider` (for example, with a React `key`). When omitted, `join()` creates a device controller internally.
