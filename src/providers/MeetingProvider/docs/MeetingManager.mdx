...
You can optionally pass a device controller as a second parameter to set up devices before joining a meeting. This is a builder-supplied controller (for example, a `DefaultDeviceController` from the Amazon Chime SDK for JavaScript). When provided, the `MeetingManager` uses this controller instead of creating one during `join()`, so device enumeration, selection, camera preview, the microphone activity meter, and permission prompts work before the meeting. The same controller then carries into the meeting and is not destroyed across `leave()` and `join()`, so your device selections persist when you leave and rejoin. The media inputs are stopped on `leave()` (releasing the camera and microphone), so restart them on rejoin (for example, with `startVideoInput`). Your application owns this controller: construct it and destroy it yourself. When omitted, the `MeetingManager` creates a device controller internally during `join()` and destroys it on `leave()`.
...
const meetingManager = new MeetingManager(logger, deviceController);
```
