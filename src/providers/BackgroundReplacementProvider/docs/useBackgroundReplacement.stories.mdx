...
to [Amazon Chime JS SDK Background replacement Guide](https://github.com/aws/amazon-chime-sdk-js/blob/main/guides/15_Background_Filter_Video_Processor.md#integrating-background-filters-into-your-amazon-chime-sdk-for-javascript-application).
...
Lastly, make sure to construct a new DefaultVideoTransformDevice using `createBackgroundReplacementDevice` if the Props of the provider were changed.
  ...
  useEffect(() => {
    async function toggleBackgroundReplacement() {
      ...
      if (isVideoTransformCheckBoxOn) {
        current = await createBackgroundReplacementDevice(device);
      } else {
        if (isVideoTransformDevice(device)) {
          let intrinsicDevice = await device.intrinsicDevice();
          device.stop();
    ...
    <div>
      {isBackgroundReplacementSupported && (
        <button onClick={onClick}>
          {isVideoTranformDevice(device) ? 'Background Replacement Enabled' : 'Enable Background Replacement'}
        </button>
      )}
    </div>
  );
